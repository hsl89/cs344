                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for (int filter_x = -filterWidth/2; filter_x <= filterWidth/2; ++filter_x){
    for (int filter_y = -filterWidth/2; filter_y <=filterWidth/2; ++filter_y){
      // image coordinate 
      // clamp if needed
      int image_x = min(max(center_x + filter_x, 0), numCols-1);
      int image_y = min(max(center_y + filter_y, 0), numRows-1);

      int image_flat_idx = image_y * numCols + image_x;
      int filter_flat_idx = (filter_y + filterWidth/2) * filterWidth + (filter_x + filterWidth/2);
      result += static_cast<float>(inputChannel[image_flat_idx]) * filter[filter_flat_idx]; 
    }
  }
  // inputChannel and outputChannel point to the same array
  // so need to sync threads before writing
  __syncthreads();
  outputChannel[center_y * numCols + center_x] = result;
